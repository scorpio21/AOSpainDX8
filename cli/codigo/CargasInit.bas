...
Public Sub DrawGrhtoHdc(ByVal hwnd As Long, ByVal Hdc As Long, ByVal grhIdx As Integer, _
    ...
    If FileSize < 54 Then
        ...
        LogError "DrawGrhtoHdc: BMP demasiado peque�o. " & _
                 "Size=" & FileSize & _
                 " GrhIdx=" & grhIdx
...
'             " FileSize=" & FileSize
    ...
    If PixelDataOffset < 0 Or PixelDataOffset >= FileSize Then
        LogError "DrawGrhtoHdc: FAIL PixelDataOffset inv�lido"
        Exit Sub
    End If
    ...
    ' La paleta real termina justo antes de los p�xeles
    colorTableSize = PixelDataOffset - 14 - 40

    If colorTableSize < 0 Then
        LogError "BMP: ERROR tama�o de paleta=" & colorTableSize
        Exit Sub
    End If
...
'             " ColorTableSize=" & colorTableSize
   ...
   Dim DrawGrh As String
    ...
    ' Para BMP de 8 bits, cada fila tiene padding para ser múltiplo de 4 bytes
    Dim rowSize As Long
...
'*****************************************************************
    ...
    ' Por tanto el primer GRH est� en Seek 274.
    '-------------------------------------------------------------
    Seek #Handle, 274

    '-------------------------------------------------------------
    ' PASADA 1
    ' Buscar el GRH m�ximo para dimensionar GrhData.
    '-------------------------------------------------------------
    maxGrh = 0
