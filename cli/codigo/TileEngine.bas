...
'Aï¿½adido soporte para mapas de 200x200
'*******************************************
...
Public Sub ClimaX()
'***************Lorwik/Climas*************************
'Descripciï¿½n: Efectos climatologicos
'*****************************************************
...
Dim i As Long
    ...
    ' [FIX] Inicializar ColorClimax al color diurno para evitar fade oscuro al entrar
    ColorClimax.r = 230
    ColorClimax.g = 200
    ColorClimax.b = 200
    ColorClimax.A = 255
...
On Error Resume Next
    ...
    ' LOG DE BOUNDS
    If LogsEnabled And Log_RenderScreen Then
        LogLevel "Render_Bounds.log", "INFO", _
            "tile=" & tilex & "," & tiley & _
            " Half=" & HalfWindowTileWidth & "," & HalfWindowTileHeight & _
            " screenMin=" & screenminX & "," & screenminY & _
            " screenMax=" & screenmaxX & "," & screenmaxY & _
            " renderer=" & frmMain.renderer.Width & "x" & frmMain.renderer.Height
    End If
    ...
    For Y = screenminY To screenmaxY
        For X = screenminX To screenmaxX
            ...
            If LogsEnabled And Log_RenderScreen Then
                LogLimited "Render_FirstTile.log", _
                    "X=" & X & _
                    " Y=" & Y & _
                    " ScreenX=" & ScreenX & _
                    " ScreenY=" & ScreenY & _
                    " Grh=" & MapData(X, Y).Graphic(1).GrhIndex, 10
            End If
            'Capa 1 **********************************
          ...
          For ZonaIndex = 0 To UBound(ZonaList)
            '15/12/2018 Irongete: Estï¿½ este tile dentro de la zona?
    ...
    '18/02/2016 Irongete: Si estï¿½ debajo de techo bajamos la transparencia a cero, si estï¿½ fuera de techo la bajamos hasta lo que tenga en la configuraciï¿½n
...
On Error GoTo Error
    ...
    If Not GrhData(Grh.GrhIndex).Active Then Exit Sub  ' [FIX] GRH inactivo ok, no loguear
    If Animate Then
...
'***************************************************
    ...
    If LogsEnabled And Log_DX8Debug Then
    LogLevel "Engine_Init.log", "INFO", _
        "Tile=" & TilePixelWidth & "x" & TilePixelHeight & _
        " Window=" & WindowTileWidth & "x" & WindowTileHeight & _
        " MainViewTop=" & MainViewTop & _
        " MainViewLeft=" & MainViewLeft & _
        " Renderer=" & frmMain.renderer.Width & "x" & frmMain.renderer.Height
    End If
